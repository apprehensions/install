...
echo -e "[Service]\nExecStart=\nExecstart=-/usr/bin/agetty --skip-login --no-newline --noissue --autologin wael\nTTYVTDisallocate=no" > /mnt/etc/systemd/system/getty@tty1.service.d/override.conf
echo -e "LANG=en_US.UTF-8" > /mnt/etc/locale.conf
sed -i '177s/.//' /mnt/etc/locale.gen
sed -i '82s/. //' /mnt/etc/sudoers
sed -i '33s/.//' /mnt/etc/pacman.conf && sed -i '36s/.//' /mnt/etc/pacman.conf && sed -i '37s/.//' /mnt/etc/pacman.conf && sed -i '37s/5/12/' /mnt/etc/pacman.conf
